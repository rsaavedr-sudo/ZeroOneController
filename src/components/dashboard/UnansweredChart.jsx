...
/**
 * UnansweredChart — distribuição das chamadas NÃO atendidas (donut).
 * data: [{ tipo, valor, cor }]
 */
export default function UnansweredChart({ data }) {
  ...
  return (
    <div className="flex flex-col gap-4 lg:flex-row lg:items-center">
        ...
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            ...
            <Tooltip
              ...
              formatter={(valor, nome) => [`${valor} chamadas`, nome]}
            />
          </PieChart>
        </ResponsiveContainer>
      ...
      <ul className="flex-1 space-y-2">
        {data.map((d) => {
          const pct = total ? ((d.valor / total) * 100).toFixed(1).replace('.', ',') : '0'
          return (
